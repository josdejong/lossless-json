...
import { createNumber } from './LosslessNumber';
...
let revive = null;                // optional reviver function
...
export function parse (text, reviver) {
  ...
  return revive ? revive('', json) : json;
}
...
function parseObject () {
  if (token == '{') {
    ...
    while (true) {
      ...
      object[key] = revive ? revive(key, value) : value;
...
function parseArray () {
  if (token == '[') {
    ...
    while (true) {
      // parse item
      let item = parseObject();
      if (revive) {
        array.push(revive(array.length + '', item));
      }
      else {
        array.push(item);
      }
...
/**
 * Parse a number. The number will be parsed as a number or LosslessNumber.
 * @return {*}
 */
function parseNumber () {
  if (tokenType == TOKENTYPE.NUMBER) {
    let number = createNumber(token);
    getToken();
    return number;
  }
